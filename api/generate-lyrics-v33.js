...
function taskForAction(action, previousLyrics) {
  if (action === "regenerate") {
    return `Write a complete new draft that is substantially different in scenes, central phrases, line shapes, and wording. Preserve only the core brief and requested structure.\n\nPREVIOUS DRAFT:\n${previousLyrics || "None"}`;
  }
  if (action === "polish") {
    return `Return a complete polished version. Preserve the core story, strongest images, section order, and emotional intent. Improve specificity, flow, natural phrasing, and line economy.\n\nCURRENT LYRICS:\n${previousLyrics || "None"}`;
  }
  if (action === "continue") {
    return `Continue without repeating existing lines. Output continuation only, beginning with an appropriate section label.\n\nCURRENT LYRICS:\n${previousLyrics || "None"}`;
  }
  if (action === "hooks") {
    return `Write exactly five distinct central-phrase options. Label them [Option 1] through [Option 5]. Each option should be 2-4 concise lines.\n\nCURRENT LYRICS FOR CONTEXT:\n${previousLyrics || "None"}`;
  }
