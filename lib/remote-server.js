...
export const REMOTE_DEFAULTS = Object.freeze({
  ...
  volumeGb: Math.max(10, Math.min(100, Number(process.env.REMOTE_VOLUME_GB) || 20)),
  estimatedHourlyUsd: Number(process.env.REMOTE_HOURLY_USD) || 0.03571,
  estimatedVolumeMonthlyUsd: Number(process.env.REMOTE_VOLUME_MONTHLY_USD) || 2
});
...
export async function ensureProfileVolume() {
  ...
  const data = await digitalOceanRequest("/volumes", {
    ...
    body: JSON.stringify({
      ...
      description: "Persistent Chrome profile and Docker cache for Forge Studio Remote",
...
if [ ! -e "$DEVICE" ]; then
  echo "Forge profile volume was not attached" >&2
  exit 1
fi
...
mkdir -p /mnt/forge-studio-profile/chromium /mnt/forge-studio-profile/tailscale /mnt/forge-studio-profile/docker
chown -R 1000:1000 /mnt/forge-studio-profile/chromium

mkdir -p /var/lib/docker
mount --bind /mnt/forge-studio-profile/docker /var/lib/docker
grep -q "/var/lib/docker none bind" /etc/fstab || echo "/mnt/forge-studio-profile/docker /var/lib/docker none bind 0 0" >> /etc/fstab
